...
def play_window(
    ...
    """
    Play ONLY the requested temporal window.

    IMPORTANT:
    OpenCV seeking (CAP_PROP_POS_MSEC / CAP_PROP_POS_FRAMES) is
    unreliable for some UCF-Crime encoded videos. Therefore this
    function does NOT trust OpenCV seeking.

    It opens the video, sequentially skips frames until the exact
    requested start time, then displays frames only until end time.
    This guarantees that a 30-40s candidate cannot accidentally
    replay from 0s.
    """
    ...
    if end <= start:
        cap.release()
        raise RuntimeError(
            f"Invalid playback window: {start:.2f}s -> {end:.2f}s"
        )
    ...
    window_name = "SENTINELAI TEMPORAL REVIEW"
    ...
    def seek_by_decoding(target_seconds):
        """
        Reliable seek: start from frame 0 and discard frames until
        target_seconds. Do not use CAP_PROP_POS_MSEC/FRAME because
        those seeks are unreliable for these videos.
        """
        ...
        # Fast path: grab() skips decoding/displaying the frame data.
    ...
    def draw_frame(frame, current_time, paused_state=False):
        overlay = frame.copy()

        cv2.putText(
            overlay,
            f"WINDOW: {start:.2f}s -> {end:.2f}s",
            (20, 40),
        ...
        cv2.putText(
            overlay,
            f"CURRENT: {current_time:.2f}s   SPEED: {speed:.1f}x",
            (20, 78),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.75,
            (0, 255, 255),
        ...
        cv2.putText(
            overlay,
            "P=Pause  R=Restart  1=1x  2=1.5x  3=2x  Q=Finish",
    ...
    try:
        # ----------------------------------------------------
        # RELIABLE INITIAL POSITIONING
        # ----------------------------------------------------
        if not seek_by_decoding(start):
            return

        # The first read after grab() is the frame at/just after start.
        while True:
            if not paused:
                ret, frame = cap.read()
                if not ret:
                ...
                # Never display anything before requested start.
                if frame_time < start:
                    continue

                # Hard stop at requested end.
                if frame_time >= end:
                    break
                ...
                if displayed_frame is None:
                    # Should only happen if pause was pressed immediately.
                    delay = 100
                else:
                    pause_time = (
                        max(0, current_frame - 1) / fps
                    )
                    cv2.imshow(
                        window_name,
                        draw_frame(
            ...
            if key in (ord("p"), ord("P")):
                ...
                # Restart THIS window, not the complete video.
                if not seek_by_decoding(start):
                    break
                paused = False
                displayed_frame = None
